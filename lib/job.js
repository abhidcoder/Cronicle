...
module.exports = Class.create({
	...
	launchLocalJob: function(job) {
		...
		// setup environment for child (uid/gid are Unix-only; skip on Windows)
		var has_uid_gid = (typeof process.getuid === 'function' && typeof process.getgid === 'function');
		var child_opts = {
		...
		if (has_uid_gid) {
			// get uid / gid info for child env vars (Unix only)
			...
			child_opts.env.USER = child_opts.env.USERNAME = process.env.USERNAME || process.env.USER || process.env.LOGNAME || 'user';
			child_opts.env.HOME = process.env.HOME || process.env.USERPROFILE || (process.env.HOMEDRIVE && process.env.HOMEPATH ? process.env.HOMEDRIVE + process.env.HOMEPATH : '') || child_opts.cwd;
		}
		...
		// make sure child can write to log file (chmod is Unix-only)
		try { fs.chmodSync( job.log_file, "777" ); } catch (e) { /* ignore on Windows */ }
		...
		if (job.detached) {
			...
			try { fs.chmodSync( temp_file, "777" ); } catch (e) { /* ignore on Windows */ }
			this.logDebug(9, "Job temp file: " + temp_file );
			child_opts.detached = true;
			child_opts.stdio = ['ignore', 'ignore', 'ignore'];
			var detached_script = path.resolve("bin/run-detached.js");
			try {
				if (process.platform === 'win32') {
					child = cp.spawn( process.execPath, [ detached_script, "detached", temp_file ], child_opts );
				} else {
					child = cp.spawn( detached_script, ["detached", temp_file], child_opts );
				}
			}
			catch (err) {
				job.pid = 0;
			...
			// Windows: run .js/.mjs/.cjs with node (no shebang execution)
			...
			try {
				child = cp.spawn( spawn_cmd, spawn_args, child_opts );
	...
	monitorServerResources: function(callback) {
		...
		// shell exec to get running process cpu and memory usage (Unix only; skip on Windows)
		var self = this;
		var now = Tools.timeNow();
		var job_startup_grace = this.server.config.get('job_startup_grace') || 5;

		if (process.platform === 'win32') {
			// No /bin/ps on Windows; skip CPU/mem from ps, still update log file sizes
