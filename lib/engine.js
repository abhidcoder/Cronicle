...
module.exports = Class.create({
	...
	checkMasterEligibility: function(callback) {
		...
		this.storage.listGet( 'global/servers', 0, 0, function(err, servers) {
			...
			var inList = !!Tools.findObject(servers, { hostname: self.server.hostname });
			if (!inList && !self.multi.cluster) {
				if (self.server.config.get('master')) {
					self.logDebug(4, "Self-registering as " + self.server.hostname + " (master: true, not in cluster)");
					self.storage.listPush( 'global/servers', { hostname: self.server.hostname, ip: self.server.ip || '127.0.0.1' }, function(pushErr) {
						if (pushErr) {
							self.logError('master', "Failed to self-register: " + pushErr.message);
							self.multi.eligible = false;
							if (callback) callback();
							return;
						}
						self.storage.listGet( 'global/server_groups', 0, 0, function(ge, groups) {
							groups = ge ? [] : (groups || []);
							var match = false;
							for (var i = 0; i < groups.length; i++) {
								if (!groups[i].master) continue;
								try {
									if (new RegExp(groups[i].regexp).test(self.server.hostname)) { match = true; break; }
								} catch (e) {}
							}
							if (!match) {
								var esc = self.server.hostname.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
								self.storage.listPush( 'global/server_groups', { id: 'local', title: 'Local', regexp: '^(' + esc + ')$', master: 1 }, function() {
									self.multi.cluster = true;
									self.multi.eligible = true;
									self.logDebug(4, "Server is eligible to become master (self-registered)");
									if (callback) callback();
								});
							} else {
								self.multi.cluster = true;
